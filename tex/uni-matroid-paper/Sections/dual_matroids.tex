...
\end{figure}
Note that, in figure (\ref{StandertRepMat}), in the matrix $[I_r|D]$ only the columns are labelled, but in $D$, both the rows and columns are labelled. Also note that, for the basis defined above $B=\{e_1, e_2,\cdots,e_r\}$ its elements correspond to columns of the identity matrix (figure (\ref{StandertRepMat})).
