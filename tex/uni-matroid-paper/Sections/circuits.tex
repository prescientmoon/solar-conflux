          ...
          \begin{enumerate}
            ...
            \begin{enumerate}
              ...
              \item The circuit $C$ is the unique circuit with the property $C \subseteq Y \cup x.$
                \begin{proof}
                  Assume another distinct $D \in \mathcal C$ exists such that $D \subseteq Y \cup x$. As $x \in D$ and $x \in C$, we can conclude that $x \in C \cap D$. 

                  We can now apply (C3) to generate some circuit $C' \subseteq  (C \cup D) - x$. We recall that $C, D \subseteq Y \cup x$, therefore $C \cup D \subseteq Y \cup x$. Furthermore, $(C \cup D) - x \subseteq Y$. By transitivity with $C' \subseteq (C \cup D) - x$ we now have $C' \subseteq Y$. This implies by definition that $Y \not\in \mathcal I$, which is a contradiction.
                \end{proof}
