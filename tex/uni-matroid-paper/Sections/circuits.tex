...
\begin{proof} We start by showing that $\mathcal I $ satisfies the necessary conditions for $(E, \mathcal I )$ to be a matroid:
    \begin{enumerate} 
        ...
        \item[(I3)] Given $X, Y \in \mathcal I $ with $|X| < |Y|$ we must show there exists some $e \in Y \setminus X$ such that $X \cup \{e\} \in \mathcal I$. Assume that a counterexample exists. That is, some pair $X, Y \in \mathcal I$ must exist such that $\forall e \in Y - X, X \cup e \not\in \mathcal I$. Pick such a counterexample which maximizes $|X \cap Y|$. Pick some $x \in X - Y$. We will proceed by cases:
          \begin{enumerate}
