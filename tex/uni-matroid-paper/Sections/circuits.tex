...
\begin{proof} We start by showing that $\mathcal I $ satisfies the necessary conditions for $(E, \mathcal I )$ to be a matroid:
    \begin{enumerate} 
        ...
        \item[(I3)] Given $X, Y \in \mathcal I $ with $|X| < |Y|$ we must show there exists some $e \in Y - X$ such that $X \cup \{e\} \in \mathcal I$. Assume that a counterexample exists. That is, some pair $X, Y \in \mathcal I$ must exist such that $\forall e \in Y - X, X \cup e \not\in \mathcal I$. Pick such a counterexample which maximizes $|X \cap Y|$. Assume $X - Y$ is nonempty (otherwise $X \subseteq Y$ and by (I2) we can pick any $Y - X$ such that $X \cup y \in \mathcal I$)  . Pick some $x \in X - Y$. We will proceed by cases:
          \begin{enumerate}
            \item If $Y \cup x$ is independent, we claim that $(X, Y \cup x)$ is a counterexample as well. This is trivially shown by noticing how $(Y \cup x) - X = Y - X$, so any choice of $e \in (Y \cup x) - X$ such that $X \cup e$ is independent would invalidate $(X, Y)$ being a counterexample. We note that 
              \begin{align*}
              |X \cap (Y \cup y)| = |(X \cap Y) \cup (X \cap \{x\})| = |(X \cap Y) \cup x|,
              \end{align*}
            ...
            \begin{enumerate}
              ...
              \item This circuit is unique
                \begin{proof}
                  Assume another distinct $D \in \mathcal C$ exists such that $D \subseteq Y \cup x$. As $x \in D$ and $x \in C$, we can conclude that $x \in C \cap D$. We can now apply (C3) to generate some $C' \subseteq  C \cup D - y$. We recall that $C, D \subseteq Y \cup y$, therefore $C \cup D \subseteq Y \cup y$. Furthermore, $C \cup D - y \subseteq Y$. By transitivity with $C' \subseteq C \cup D - y$ we now have $C' \subseteq Y$, which implies $C' \not\in \mathcal I$, hence a contradiction.
                \end{proof}
              ...
              \item $Y \cup x - y$ is independent. 
                ...
                \begin{proof}
                If this is not the case, there must exist some $D \subseteq \mathcal C$ with $D \subseteq Y \cup x - y$. As $D$ does not contain $x$, it must be distinct from $C$. We note that $Y \cup y - x \subseteq Y \cup x$, so $D \in Y \cup x$ as well. This contradicts the uniqueness of $C$, hence a contradiction.
                \end{proof}
