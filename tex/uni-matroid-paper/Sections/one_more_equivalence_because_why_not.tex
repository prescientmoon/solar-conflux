...
\begin{enumerate}
    ...
    \item If $X \subseteq E$ is arbitrary then we have that $f(f(X))$ is the minimal member of $\caf$ containing $f(X)$, but $f(X)$ is by definition itself in $\caf$, and it contains $f(X)$. So we have that $f(X) \subseteq f(f(X)) \subseteq f(X)$ implying $f(f(X)) = f(X).$
