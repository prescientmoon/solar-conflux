...
\end{enumerate}
  ...
  Let $\mathcal C'$ be the set of circuits in our newly defined matroid. It then remains to prove that $\mathcal C = \mathcal C'$. We already know from lemma (\ref{lem:circuits-have-properties}) that $\mathcal C' \subseteq \mathcal C$. It remains to prove that $\mathcal C \subseteq \mathcal C'$. This is equivalent to proving that satisfying (C1), (C2) and (C3) is enough to be considered a circuit. 

  Assume a subset $C$ with these properties exists which is not a circuit. Clearly, $C \not\in \mathcal I$ is implied by the definition of $\mathcal I$ seen above. This implies that $C$ is dependent but not a circuit, which means $C$ is not miminal. 

  A proper subset $D  \subsetneq C$ must then exist such that $D$ is a circuit. We know from lemma (\ref{lem:circuits-have-properties}) that $D \in \mathcal C$. We have now constructed a contradiction to (C2), hence our assumption that such a circuit exists is false.
