or not, the problem is her comments are lost, i will rename this an add graphs in here
...
It remains to prove that $\mathcal C $ is the set of circuits in the newly defined matroid. We start by proving that all circuits have these properties:
\begin{enumerate}
...
\end{enumerate}
    ...
    It now remains to prove that having these properties is enough to be considered a circuit. Assume a subset $C$ with these properties exists which is not a circuit. Clearly, $I \not\in \mathcal I$ is implied by the definition of $\mathcal I$ seen above. This implies that $C$ is dependent but not a circuit, which means $C$ is not miminal. A proper subset $D  \subsetneq C$ must then exists such that $D$ is a circuit. As proven above, $D$ satisfies the three axioms, hence $D \in \mathcal C$. We have now constructed a contradiction to (C2), hence our assumption that such a circuit exists is false.
