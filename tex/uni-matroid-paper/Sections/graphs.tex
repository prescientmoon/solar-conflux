...
\end{subfigure}
   ...
   Second, assume there exists $e \in C_1 \cap C_2$. Let the cycle $C_1$ correspond to the sequence of incident vertices  $v_0, \cdots, v_n = v_0$ and let (without loss of generality) the special edge in the intersection be the edge $e = \{v_0, v_1\}$. The idea of the proof is very simple. Our goal is to find a cycle in the set of edges $C_1 \cup C_2 -e$, which is not difficult. We go around $C_1$ until we can `jump' to $C_2$ for the first time, then we go back to $e$ via $C_2$ from the `backward direction'.
