...
\label{impostor2}
   ...
   As an illustration of the procedure we see a graph in figure (\ref{impostor2}). The vertex $v_4$ is the first one not in $C_2$ and $v_9$ is the first one in $C_1$ which is after $v_4$ but is back in $C_2$. The cycle obtained in this procedure is $C_3$ which evidently does not include $e.$
    ...
    Let $k$ be the smallest integer $1<k< n$ such that the vertex $v_k$ is not in the cycle $C_2$. We know that such an integer exists because we have already proved (C2), so if $C_1$ and $C_2$ are distinct, then necessarily one is not contained in the other, which means both $C_2 - C_1$ and $C_1 -C_2$ are nonempty.
