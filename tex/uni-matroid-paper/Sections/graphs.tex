...
In figure \ref{ilustratinloops} we see graphs $G$ and $H$, the first one has loops and the second one has parallel edges. One way to make both constructions formal is to use \textit{multiset} of edges instead of a set as in \cite{oxley1}[4]

However, the formal construction is not so important, as we will see in the future and one can usually restrict to simple graphs.  However, we are interested in \textit{properties} loops and parallel edges have.

For clarity, we define exacly what we mean by a cycle in a finite graph.
