...
Before we proceed, we will outline a few examples of risk factors commonly used for statistical profiling that fall on the different sides of each criteria above. For the first criteria, age is a risk factor the agent is not responsible for, while their level of physical activity is one they usually are. For the second criteria, people who smoke are more likely to attain certain diseases (todo: add source), leading to higher medical costs (todo: insert source), thus whether the agent smokes is a risk factor that is causally responsible for the increased health costs. On the other hand, a person's place of living could be used for profiling if correlation was observed with health costs, all without requiring the causality link to exist (as mentioned in~\cite{odihi20}). As for the last criteria, while people in developed countries can, in the 21st century, be reasonably expected to know about the health risks of smoking (insert source), this would not have been the case a hundred years ago (insert source).
