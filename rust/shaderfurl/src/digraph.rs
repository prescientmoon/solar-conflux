...
pub trait Dependencies<K> {
	fn dependencies(&self) -> impl Iterator<Item = K>;
}
	...
	V: Dependencies<K>,
{
		...
		if let Some(node_value) = graph.get(node_key) {
			for dep_key in node_value.dependencies() {
				...
				match states.get(&dep_key).unwrap_or(&NodeState::Unvisited) {
					...
					NodeState::Unvisited => {
						dfs_visit(&dep_key, graph, states, sorted_list)
					}
					...
					NodeState::Visiting => {
						sorted_list
							.push(NodeStatus::CycleBreaker(node_key.clone()));
						states
							.insert(node_key.clone(), NodeState::VisitedEarly);
						return;
					}
	...
	// We go over everything in order to handle disconnected components
	...
	fn assert_valid_sort<K: Eq, V: Dependencies<K>>(
		...
		for (k, v) in graph.iter() {
			...
			for dep in v.dependencies() {
				let found_dep =
					sorted.iter().take(pos).any(|node| node.key() == &dep);
				assert!(found_dep);
			}
		}
	}
