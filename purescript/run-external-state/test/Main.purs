...
import Run.State.External (get, imapExternalState, put, runExternalStatePure, runExternalStateUsingRef)
...
spec = describe "External state" do
  ...
  describe "imapExternalState" do
    it "should allow mapping over the state" do
      let initial = 13.0
      let other = 20
      let computation = imapExternalState floor toNumber do
           state <- get
           put other
           pure (state * 2)
      let final = extract $ runExternalStatePure initial computation
      final `shouldEqual` (20.0 /\ 26)
  describe "runExternalStateUsingRef" do
